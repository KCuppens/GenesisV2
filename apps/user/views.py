...
def register(request,
    ...
    if registered_user_redirect_to is None:
        registered_user_redirect_to = get_config('LOGIN_ADMIN_REDIRECT_URL')

    if request.user.is_authenticated:
            return redirect(registered_user_redirect_to)

    if not get_config('USERS_REGISTRATION_OPEN'):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            if get_config('USERS_AUTO_LOGIN_AFTER_REGISTRATION'):
                user.backend = 'django.contrib.auth.backends.ModelBackend'
                login(request, user)
            elif not user.is_active and get_config('USERS_VERIFY_EMAIL'):
                opts = {
                    'user': user,
                    'request': request,
...
def registration_complete(request,
                          ...
                          extra_context=None):
    context = {
        'login_url': resolve_url(get_config('LOGIN_URL')),
...
def activate(request,
    ...
    if user is not None and token_generator.check_token(user, token):
        ...
        if get_config('USERS_AUTO_LOGIN_ON_ACTIVATION'):
