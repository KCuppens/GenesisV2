...
if get_config('USERS_CREATE_SUPERUSER'):
    try:
...
def auto_create_superuser(sender, **kwargs):
    if not get_config('USERS_CREATE_SUPERUSER'):
        return

    email = get_config('USERS_SUPERUSER_EMAIL')
    password = get_config('USERS_SUPERUSER_PASSWORD')
...
class EmailActivationTokenGenerator(object):
    ...
    def check_token(self, user, token):
        ...
        if (self._num_days(self._today()) - ts) > get_config('USERS_EMAIL_CONFIRMATION_TIMEOUT_DAYS'):
            return False
...
def send_activation_email(
    ...
    if not user.is_active and get_config('USERS_VERIFY_EMAIL'):
        ...
        context = {
            'email': user.email,
            'site': current_site,
            'expiration_days': get_config('USERS_EMAIL_CONFIRMATION_TIMEOUT_DAYS'),
            'user': user,
