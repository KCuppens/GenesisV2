...
from .settings import (
    get_available_backends, get_batch_size, get_log_level, get_max_retries, get_retry_timedelta, get_sending_order, get_threads_per_process,
)
from apps.mail.models import MailTemplate, Email
...
def create(sender, recipients=None, cc=None, bcc=None, subject='', message='',
    ...
    if render_on_delivery:
        ...
        if template:
            ...
            message = template.content_plain
            html_message = template.content_html
...
def send(recipients=None, sender=None, template=None, context=None, subject='',
    ...
    if sender is None:
        sender = get_config('DEFAULT_FROM_EMAIL')
    ...
    if template:
        # template can be an MailTemplate instance or name
        if isinstance(template, MailTemplate):
            template = template
    ...
    if attachments:
        attachments = create_attachment(attachments)
...
def get_mail_template(keyname):
    """
    Function that returns an email template instance, from cache or DB.
    """
    return MailTemplate.objects.filter(key_name=keyname).first()
